...
void glwidget::loadPoints() {
  ...
  std::cout << "1" << std::endl;
  glVertexPointer(3, GL_FLOAT, 0, _contr.GetPoint());
  std::cout << "2" << std::endl;
  glColor3f(DotsColor.redF(), DotsColor.greenF(), DotsColor.blueF());
  glPointSize(dotsSize);
  glDrawArrays(GL_POINTS, 0, _contr.get_dots_count());
}
void glwidget::loadFacets() {
  ...
  std::cout << "3" << std::endl;
  glDrawElements(GL_LINES, _contr.get_facets_count(), GL_UNSIGNED_INT,
                 _contr.GetFacet());
  std::cout << "4" << std::endl;
}
